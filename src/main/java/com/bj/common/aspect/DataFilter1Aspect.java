...
import com.bj.common.annotation.DataFilter1;
...
@Aspect
@Component
public class DataFilter1Aspect {
    @Autowired
    ...
    @Pointcut("@annotation(com.bj.common.annotation.DataFilter1)")
    ...
    public void dataFilter(JoinPoint point) throws Throwable {
        ...
        if(params != null && params instanceof Map){
            ...
            //如果不是超级管理员，则进行数据过滤
            if(user.getUserId() != CommonEnum.SUPER_ADMIN){
                Map map = (Map)params;
                map.put(CommonEnum.SQL_FILTER, getSQLFilter(user, point));
            }
    ...
    private String getSQLFilter(SysUserEntity user, JoinPoint point){
        ...
        DataFilter1 dataFilter = signature.getMethod().getAnnotation(DataFilter1.class);
        //获取表的别名
        ...
        if(deptIdList.size() > 0) {
            sqlFilter.append(tableAlias).append(dataFilter.orgCode()).append(" in(select org_code from sys_dept where ")
